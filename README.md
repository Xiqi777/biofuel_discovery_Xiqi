...
2. **Classifier Development**
   - Use XGBoost and CATBoost to run the scripts `classifier_Xgboost_fingerprint_biofuel.py` and `CATboost_classifier_biofuel.py` to gain the fingerprints of molecules in `seed_dataset.csv`, and create classifiers that can differentiate between biofuels and non-biofuels.
...
4. **Creating a Biofuel Dataset**
   - Combine the biofuel SMILES from `seed_datasets.csv` and the SMILES extracted in step 4 to create a biofuel database named `seed_biofuels_from_Lotus.csv`, which will be provided to MOLGAN.
...
This code snippet creates an output directory named CATBoost_output to store the generated output files. You can change the output path by modifying the output_dir part.
...
This function reads a CSV file and checks if it contains the 'SMILES' and 'Type' columns. The parameter `csv_file` is the path to the CSV file.
...
This function converts a SMILES string into a molecular fingerprint. The radius of the fingerprint can be changed by modifying `radius`, and the number of bits can be changed by modifying `n_bits`.
...
This function converts the SMILES strings in the dataset to fingerprints and returns the feature matrix `X`, the labels `y`, and the array of `SMILES` strings smiles.

```bash
# Apply fingerprint function to dataset
def apply_fingerprint(df):
    print("Applying fingerprint conversion...")
    df['Fingerprint'] = df['SMILES'].apply(smiles_to_fingerprint)
    X = np.stack(df['Fingerprint'])
...
This function splits the data into training and testing sets. The test set proportion can be changed by modifying `test_size`, and the random seed can be set with `random_state` to ensure reproducibility.
...
This function filters out low variance features using a specified variance threshold. The threshold can be changed by modifying `threshold`.

```bash
# Low variance filter function
def low_variance_filter(X, threshold=0.01):
    selector = VarianceThreshold(threshold=threshold)
    X_filtered = selector.fit_transform(X)
    assert X_filtered.shape[1] > 0, "All features removed by low variance filter"
...
This function filters out highly correlated features using a specified correlation threshold. The threshold can be changed by modifying `threshold`.
...
This function generates molecular fingerprints with different radii and bit counts. You can change the `radius` and `n_bits` parameters to generate different fingerprints.

```bash
# Function to manipulate fingerprints
def manipulate_fingerprints(smiles, radius=3, n_bits=4096):
    return np.array([smiles_to_fingerprint(s, radius=radius, n_bits=n_bits) for s in smiles])
```

This function combines three preprocessing methods: low variance filtering, correlation filtering, and fingerprint manipulation. You can control whether to apply these preprocessing steps by setting `apply_low_variance`, `apply_correlation_filter`, and `apply_fingerprint_manipulation`.
...
This function calculates metrics such as accuracy, ROC AUC, F1 score, precision, recall, confusion matrix, and classification report.
...
def compute_metrics(y_true, y_pred, y_proba):
    ...
    metrics = {
        'accuracy': accuracy_score(y_true, y_pred),
        'roc_auc': roc_auc_score(y_true_bin, y_proba),  # Compute AUC using binary labels
...
This function plots and saves the ROC curve. You can change the file name by modifying the `filename` parameter.
...
This function plots a pairplot of important features. You can change the file name by modifying the `filename` parameter.

```bash
# Function to plot pairplot of important features
def plot_pairplot(df, features, target_column, filename='pairplot.png'):
    sns.pairplot(df[features + [target_column]], hue=target_column)
    plt.savefig(os.path.join(output_dir, filename))
    plt.close()
...
This function trains a baseline model without preprocessing. You can adjust the training process by modifying the parameters of `CatBoostClassifier`.
...
This function uses TPOT for hyperparameter optimization. You can change the optimization process by modifying `generations`, `population_size`, and `cv`.
...
This function retrieves the most important features from the model. You can change the number of important features extracted by modifying the `top_n` parameter..
...
Reads a CSV file (`seed_biofuels_from_Lotus.csv`) into a Pandas DataFrame (`df`) and sets a variable `num_atoms` to 12. You can change the filename (`seed_biofuels_from_Lotus.csv`) or adjust `num_atoms` to handle different molecule sizes.

```bash
df = pd.read_csv("seed_biofuels_from_Lotus.csv")
num_atoms = 12
```
